...
function App() {
  ...
  const [activeTab, setActiveTab] = useState('perfil'); // 'perfil', 'jugar', 'fixture'
  ...
  return (
    ...
    <main className="container">
      ...
      {/* Tab: JUGAR */}
      <div className={`tab-content ${activeTab === 'jugar' ? 'active' : ''}`}>
      ...
      {/* Tab: FIXTURE */}
      <div className={`tab-content ${activeTab === 'fixture' ? 'active' : ''}`}>
        <div className="col-fixture">
          <PanelFixture nroFecha={nroFecha} />
        </div>
        <div className="col-derecha mt-3">
          <TablaPremios
            estadoSorteo={estadoSorteo}
            aciertos={aciertos}
          />
      ...
      {/* Modales globales (quedan fuera de los tabs) */}
      {showModalCompra && (
        <CompraFichasModal
    ...
    <nav className="mobile-tab-bar">
      <button 
        className={`mobile-tab-btn ${activeTab === 'perfil' ? 'tab-active' : ''}`} 
      ...
      <button 
        className={`mobile-tab-btn ${activeTab === 'jugar' ? 'tab-active' : ''}`} 
        onClick={() => setActiveTab('jugar')}
      >
        <span className="tab-icon">🎟️</span>
        <span className="tab-label">Jugar</span>
      </button>
      <button 
        className={`mobile-tab-btn ${activeTab === 'fixture' ? 'tab-active' : ''}`} 
        onClick={() => setActiveTab('fixture')}
      >
        <span className="tab-icon">📅</span>
        <span className="tab-label">Fixture</span>
      </button>
    </nav>
    </>
  );
